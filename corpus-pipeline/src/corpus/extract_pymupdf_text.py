...
from config.paths import RAW_DIR, TEXT_INTERIM_DIR
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Extract DTQGVN text with PyMuPDF.")
    parser.add_argument("--pdf", type=Path, default=DEFAULT_PDF)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--pages", help="1-based pages, for example 37,99-101")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    with fitz.open(args.pdf) as doc:
        pages = parse_page_selection(args.pages, doc.page_count)
    output = process_pdf(args.pdf, args.output, pages=pages)
