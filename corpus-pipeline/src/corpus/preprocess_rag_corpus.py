...
from config.paths import RAG_INTERIM_DIR, TEXT_INTERIM_DIR
from corpus.clean_markdown_corpus import SPLIT_WORDS_REPAIRS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Preprocess page-marked Markdown into RAG-ready sections, chunks, and audit output.",
    )
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--max-chars", type=int, default=DEFAULT_CHUNK_MAX_CHARS)
...
def main() -> None:
    ...
    if args.max_chars < 200:
        raise SystemExit("--max-chars must be >= 200")
    paths = process_corpus(
        input_path=args.input, output_dir=args.out, max_chars=args.max_chars
    )
    print(paths["sections"])
    print(paths["chunks"])
    print(paths["audit"])
