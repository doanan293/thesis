...
from config.paths import ANKHANG_MARKDOWN_INTERIM_DIR, RAG_FINAL_DIR, RAW_DIR
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Integrate parsed An Khang pages.")
    ...
    parser.add_argument("--output-sections", type=Path, required=True)
    parser.add_argument("--output-chunks", type=Path, required=True)
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    sections, chunks = integrate_ankhang_corpus(
        markdown_dir=args.markdown_dir,
        sections_path=args.sections,
        chunks_path=args.chunks,
        mappings_path=args.mappings,
        output_sections_path=args.output_sections,
        output_chunks_path=args.output_chunks,
    )
    print(f"new_sections={sections}")
