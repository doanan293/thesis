...
from config.paths import PROCESSED_EVALUATION_DIR, RAG_FINAL_DIR
...
from rag_metadata.payload_layers import compact_colloquial_mapping

DEFAULT_SECTIONS_PATH = RAG_FINAL_DIR / "sections.jsonl"
DEFAULT_CHUNKS_PATH = RAG_FINAL_DIR / "chunks.jsonl"
DEFAULT_OUTPUT_PATH = PROCESSED_EVALUATION_DIR / "section_retrieval_eval.csv"
...
SECTION_CATEGORY_RULES = [
    ...
    ("adr", ("tác dụng không mong muốn", "adr"), "medium", "prose"),
    ("interaction", ("tương tác",), "medium", "prose"),
    ("pregnancy", ("mang thai", "cho con bú"), "medium", "prose"),
    (
        "general_guidance",
        ("kê đơn", "người cao tuổi", "sử dụng thuốc", "ngộ độc"),
        "medium",
        "prose",
    ),
    ("atc_appendix", ("atc", "phân loại thuốc"), "hard", "appendix_list"),
    ("brand_lookup", ("biệt dược",), "medium", "index_entry"),
]
...
def category_for_section(section: dict, chunks: list[dict]) -> tuple[str, str, str]:
    ...
    if section.get("content_type") == "general_monograph":
        return (
            "general_guidance",
            "medium",
            chunk_role_for_section(section["id"], {section["id"]: chunks}) or "prose",
        )
    return (
        "drug_fact",
        "medium",
...
def make_row(
    ...
    return {
        ...
        "expected_section_ids": "|".join(all_expected_ids),
        "query_intent_count": str(len(all_expected_ids)),
        "answer_mode": answer_mode,
        "expected_title": section_label(section),
        "expected_chunk_id": expected_chunk_id,
        "expected_chunk_index": str(expected_chunk_index),
...
CLINICAL_TEMPLATES = {
    ...
    "interaction": [
        ...
        "Khi phối hợp điều trị có {title}, cần lưu ý tương tác thuốc nào?",
    ],
    "pregnancy": [
...
class EvalRowAccumulator:
    ...
    def add(self, row: dict, prefer_existing: bool = False) -> bool:
        ...
        section_key = row["expected_section_ids"]
...
def sections_by_title_and_category(
    ...
    for item in section_items:
        ...
        if section.get("content_type") != "drug_monograph":
            continue
        title = normalize_spaces(section.get("title", ""))
        if title:
...
def generate_rows(
    ...
    ankhang_template_target = quotas["ankhang"] - ankhang_alias_target
    ...
    multi_candidates = spread_items(
        multi_section_candidates(formulary_section_items), multi_target
    )
    multi_count = 0
    multi_index = 0
    while multi_count < multi_target and multi_candidates:
        primary_item, secondary_item, template = multi_candidates[
            multi_index % len(multi_candidates)
        ]
        primary_section, _primary_category, _primary_difficulty, primary_role = (
            primary_item
        )
        (
            secondary_section,
            _secondary_category,
            _secondary_difficulty,
            _secondary_role,
        ) = secondary_item
        title = normalize_spaces(primary_section.get("title", ""))
        query = unique_query(
            template.format(title=title), primary_section, accumulator.seen_queries
        )
        row = make_row(
            f"multi-intent-{multi_count + 1:04d}",
            query,
            "multi_intent",
...
def row_eval_groups(row: dict) -> set[str]:
    ...
    groups.update(tag for tag in row.get("eval_tags", "").split("|") if tag)
...
def validate_rows(
    ...
    for index, row in enumerate(rows, start=1):
        ...
        query_pair = (row["query"].lower(), row["expected_section_ids"])
        ...
        expected_ids = row["expected_section_ids"].split("|")
...
def write_csv(path: Path, rows: list[dict]) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=EVAL_HEADER, lineterminator="\n")
        writer.writeheader()
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build section-level retrieval evaluation CSV."
    )
