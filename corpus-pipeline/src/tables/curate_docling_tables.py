...
from config.paths import DOCLING_INTERIM_DIR, RAG_INTERIM_DIR
from corpus.clean_markdown_corpus import (
    OCR_TEXT_REPAIRS,
    SPLIT_WORDS_REPAIRS,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Curate raw Docling table output.")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--sections", type=Path, required=True)
    parser.add_argument("--continuation-overrides", type=Path, default=None)
    args = parser.parse_args()
    paths = process_table_curation(
        input_dir=args.input,
        sections_path=args.sections,
        continuation_overrides_path=args.continuation_overrides,
    )
