...
from config.paths import DOCLING_INTERIM_DIR, RAW_DIR, TEXT_INTERIM_DIR
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Discover and extract PDF tables with Docling."
    )
    parser.add_argument("--pdf", type=Path, default=DEFAULT_PDF)
    parser.add_argument("--cleaned-md", type=Path, default=DEFAULT_CLEANED_MD)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument(
        "--pages",
        help="Comma-separated pages or ranges to extract, e.g. 55,58,62 or 55-62. Overrides discovery mode.",
    )
    parser.add_argument(
        "--caption-pages-only",
        action="store_true",
        help="Only scan pages with table captions found in cleaned Markdown. Default scans all PDF pages.",
    )
    parser.add_argument(
        "--ocr",
        choices=("off", "on"),
        default="off",
        help="Use Docling OCR. Defaults to off because this PDF has a text layer.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=50,
        help="Maximum contiguous pages per Docling conversion. Defaults to 50.",
    )
...
def main() -> None:
    ...
    if not args.cleaned_md.exists():
        raise SystemExit(f"Cleaned Markdown not found: {args.cleaned_md}")
    ...
    try:
        paths = process_tables(
            pdf_path=args.pdf,
            cleaned_markdown_path=args.cleaned_md,
            output_dir=args.out,
            pages=pages,
            ocr_enabled=args.ocr == "on",
            caption_pages_only=args.caption_pages_only,
