from corpus_pipeline.runtime.benchmarking import (
    BenchmarkLevel,
...
from corpus_pipeline.runtime.model_profiles import EmbeddingWorkloadProfile


def test_stratified_sample_is_stable_and_spans_lengths():
    items = [{"id": i, "length": i} for i in range(1, 101)]
    first = stratified_sample(items, 20, key=lambda x: x["id"], length=lambda x: x["length"])
    second = stratified_sample(list(reversed(items)), 20, key=lambda x: x["id"], length=lambda x: x["length"])
...
def test_embedding_levels_are_cartesian_product():
    ...
    assert embedding_levels(profile) == (
        BenchmarkLevel(4, 1), BenchmarkLevel(4, 2),
        BenchmarkLevel(8, 1), BenchmarkLevel(8, 2),
    )
