"""Query embedder for evaluation, backed by the `pharma-lab embed queries` cache.
    ...
    async def embed(self, texts: Sequence[str]) -> list[list[float]]:
        ...
        if missing:
            raise QueryEmbeddingMissing(
                ...
                f"`uv run pharma-lab embed queries --model {self._model}` for this evaluation"
            )
    ...
    *,
    ...
    if not subset.is_complete or subset.sha256 is None:
        raise QueryEmbeddingMissing(
            ...
            f"`uv run pharma-lab embed queries --model {model}` for this evaluation"
        )
