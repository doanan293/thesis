...
from pharma_lab.evaluation.artifact_contracts import ArtifactContractError
from pharma_lab.evaluation.metrics_artifacts import report_dir
    ...
    for name in MATCHING_REPORT_FIELDS:
        if baseline_identity.get(name) != candidate_identity.get(name):
            raise ArtifactContractError(
                ...
                "run pharma-lab metrics for both rerankers on the same run"
            )
