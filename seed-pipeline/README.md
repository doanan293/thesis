...
Công cụ offline tạo dữ liệu gốc cho backend `pharma-agent` từ nguồn nội bộ: **Dược thư Quốc gia Việt Nam** (PDF) và tờ hướng dẫn sử dụng thuốc (HTML đã crawl). Đầu ra là một knowledge bundle `knowledge-bundle/v1`; backend nạp bundle bằng `pharma-agent corpus import`. seed-pipeline không ghi thẳng vào Postgres hay Qdrant.
...
```text
seed source crawl      sitemap -> data/sources/leaflets/ (html, danh sách URL, manifest)
seed build             PDF + HTML tờ hướng dẫn -> data/corpus/rag-final/ (sections + blocks)
seed validate          kiểm tra contract rag-final-v3
seed bundle export     rag-final -> data/corpus/formulary/ (knowledge-bundle/v1)
seed bundle embed      chunk_section của backend -> embeddings/<model_slug>.jsonl (Kaggle hoặc local)
pharma-agent corpus import ../seed-pipeline/data/corpus/formulary --collection formulary --publish   (chạy trong backend/)
seed evaluation build  bundle -> bộ câu hỏi gold data/evaluation/gold/ (section key)
seed embed queries     bộ gold -> cache vector query (Kaggle hoặc local)
seed retrieve          RetrievalService của backend -> data/evaluation/runs/<run>/candidates/
seed rerank            chấm reranker trên candidates của run -> rerank/<model>/ (local hoặc Kaggle)
seed metrics           Hit@K, MRR -> reports/baseline/ hoặc reports/rerank/<model>/, thư mục top<K>-window<N>/
```
...
- PDF Dược thư, HTML tờ hướng dẫn và các file nguồn nhỏ (mappings, glossary, curated tables, danh sách âm tiết) dưới `data/sources/`; lần crawl đầu cần `seed source crawl --sitemap-url`.
